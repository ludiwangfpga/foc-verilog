	  ...
	  begin:FIFO_GEN
	    ...
	    begin
	      if (fifo_wren)// && S_AXIS_TSTRB[byte_index])
            ...
            Vd <=S_AXIS_TDATA[23:16];
            Vq <= S_AXIS_TDATA[31:24];
             sin_theta <= sin_table[Theta];
             cos_theta <= cos_table[Theta];
             Valpha <= (Vd * cos_theta - Vq * sin_theta) >>> 16;
             Vbeta <= (Vq * cos_theta + Vd * sin_theta) >>> 16;
             s3vb <= Vbeta * SQRT3C;
             Va <= Valpha;
             Vb <= ((s3vb >> 15) - Valpha) >> 1;
             Vc <= (0 - Valpha - (s3vb >> 15)) >> 1;
             va_calculated <= 1'b1; // 假? Va ?算已完成
             vb_calculated <= 1'b1; // 假? Vb ?算已完成
             vc_calculated <= 1'b1; // 假? Vc ?算已完成  
             stream_data_fifo[write_pointer] <= {Theta, Vc, Vb, Va}; 
	       end else begin
	        va_calculated <= 1'b0; // 假? Va ?算已完成
            vb_calculated <= 1'b0; // 假? Vb ?算已完成
            vc_calculated <= 1'b0;        
	        end  
	    end  
	  end		
